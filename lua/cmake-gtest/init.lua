...
------------------------------------------------------------------------------------------------------------------------
  ...
  table.insert(fn, function() gtest.run_testsuite({ bang = false }, { "--gtest_filter=" .. filter[1] }) end)
  table.insert(fn, function() gtest.run_testsuite({ bang = false }, { "--gtest_filter=" .. filter[2] }) end)
  table.insert(fn, function() gtest.run_testsuite({ bang = true }, { "--gtest_filter=" .. filter[3] }) end)
  table.insert(fn, function() gtest.run_testsuite({ bang = true }, { "--gtest_filter=" .. filter[4] }) end)
...
------------------------------------------------------------------------------------------------------------------------
  ...
  if (args and args.bang) then preselection = 3 end
...
------------------------------------------------------------------------------------------------------------------------
  ...
  if (args and args.bang) then preselection = 4 end
  gtest.code_action({ preselection = preselection })
end
...
function gtest.run(target, path, args, debug)
  if debug and has_nvim_dap then
    -- local opt = {}
    -- table.insert(opt, target)
    -- table.insert(opt, args)
    -- print(opt[1])
    -- print(opt[2])
    -- for i=1,#args do
    --   t1[#t1+1] = t2[i]
    ...
    cmake.quick_build({ fargs = { target } }, function ()
      ...
      local cmd = vim.fn.fnamemodify(path, ":t")
      -- always run tests from their bin dir?
      return quickfix.run(cwd, cmd, {}, args, const)
    end)
  end
end
...
------------------------------------------------------------------------------------------------------------------------
  gtest.find_testsuites(function (testsuites)
    if testsuites == nil then return end
...
function gtest.select_and_run_test(args)
  ...
  gtest.find_all_tests(function (testsuites)
    ...
    local target, test, display, path = {}, {}, {}, {}
    ...
    vim.ui.select(display, { prompt = "select test to run" }, function(_, idx)
      ...
      return gtest.run(target[idx], path[idx], { "--gtest_filter=" .. filter }, debug)
    end)
  end)
end
...
------------------------------------------------------------------------------------------------------------------------
  ...
  gtest.find_all_tests(function (testsuites)
    ...
    vim.ui.select(display, { prompt = "Select Test to run" }, function(_, idx)
      ...
      return gtest.run(suite[idx], path[idx], { "--gtest_filter=" .. display[idx] .. "*" }, debug)
    end)
  end)
end
  ...
  gtest.find_testsuites(function (testsuites)
    ...
    for idx, v in ipairs(names) do
      if table_contains(files[idx], currentfile) then
        ...
        table.insert(possiblePaths, paths[idx])
      end
    end
    ...
    elseif num == 1 then
      ...
      vim.ui.select(possibleSuites, { prompt = "Select Testsuite to run" }, vim.schedule_wrap(function (_, idx)
        ...
        return gtest.run(possibleSuites[idx], possiblePaths[idx], cmd_args or {}, debug)
      end))
    end
...
------------------------------------------------------------------------------------------------------------------------
  cmake.get_cmake_launch_targets(function (targets_res)
    ...
    if mainFiles == nil then return end
